...
const EditBonusCard = ({route}) => {
  ...
  const [inputs, setInputs] = useState({
    MaxPoint: String(item.maxPoints) || '',
    description: item.details || '',
  });
  const [AddQRCodeList, setAddQRcodeList] = useState(item.qrCodes || []);
  ...
  const UpdateBonusCard = async () => {
    try {
      ...
      const raw = JSON.stringify({
        ...
        validUntil: validUntil || item.validUntil,
      ...
      const response = await fetch(
        'https://www.mein-vendoapp.de:3103/bc_mb-admin/update_card',
        requestOptions,
      );
      const result = await response.json();
      console.log(result,"===========result");
      if(response.ok) {
        Alert.alert("Update","Loyalty card updated successfully")
      }
    } catch (error) {
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        ...
        <View style={{marginVertical: hp(0.4)}}>
          ...
          <Input
            ...
            value={
              item.validUntil
                ? moment.unix(item.validUntil).format('DD/MM/YYYY')
                : ''
            }
          ...
          AddQRCodeList.map((qr, index) => (
            <View key={index} style={styles.qrCodeContainer}>
              ...
              <Input
                ...
                onChangeText={text => {
                    let newList = [...AddQRCodeList];
                    let updatedQR = { ...newList[index], status: text };
                    newList[index] = updatedQR; 
                    setAddQRcodeList(newList); 
                }}
                />
              <Input
                ...
                value={String(qr.points)} 
                keyboardType="numeric"
                onChangeText={text => {
                  let newList = [...AddQRCodeList];
                  newList[index].points = parseInt(text) || 0;
                  setAddQRcodeList(newList);
                }}
              />
              <Input
                ...
                value={
                  qr.expiryDate
                    ? moment(qr.expiryDate).format('DD/MM/YYYY')
                    : ''
                }
        ...
        <TouchableOpacity style={styles.bonuscardBtn} onPress={UpdateBonusCard}>
          <Text
            style={{
              fontFamily: fonts.PoppinsMedium,
              fontSize: RFValue(14),
