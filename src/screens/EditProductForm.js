...
const EditProductForm = ({route}) => {
  ...
  const [Deactivate, setDeactivate] = useState(item.deactivate);
    ...
    try {
      ...
      if (ImageUrl1) {
        formdata.append('productImage2', {
          uri: ImageUrl1,
          type: mime.getType(ImageUrl1),
          name: ImageUrl1.split('/').pop(),
        });
      } else {
      ...
      const response = await fetch('https://www.mein-vendoapp.de:3001/products_mb/updateProduct',requestOptions);
    ...
    <View style={styles.container}>
      <ScrollView showsVerticalScrollIndicator={false}>
        ...
        <TouchableOpacity
          ...
          disabled={loading}>
          <Text style={styles.addProductBtnText}>Edit Product</Text>
        </TouchableOpacity>
        <BottonSheet
          isVisible={isVisible}
